...
export default function NonDuplicateDialog({ selectedResponses }: { selectedResponses: Response[] }) {
  ...
  const handleSubmit = () => {
    ...
    return toast({
      title: "Changes saved!",
      description: "Marked as non-duplicate!",
    });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={toggleOpen}>
      <DialogTrigger asChild>
        <Button variant="outline">Mark as Not Duplicate</Button>
      </DialogTrigger>
      {<DialogContent className="max-h-screen overflow-y-auto sm:max-w-[600px]">
        <DialogTitle> Mark as Not Duplicate - Confirmation </DialogTitle>
            <div className="grid w-full items-center gap-4">
              <TypographyP>Are you sure you want to mark {selectedResponses?.length || 0} entry as not a duplicate?</TypographyP>
